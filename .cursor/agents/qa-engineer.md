...
用户确认话术（固定原文）：`测试已通过，可发布`（仅在本 subagent 验收全部通过后提示；**不是** `技术设计已确认，可开始编码`）。完整状态表见 `.cursor/rules/7ai-club-workflow.mdc` §门禁状态。
...
按 **门禁状态** 提示（与 `.cursor/rules/7ai-club-workflow.mdc` 一致）：

> 测试已通过，状态：**待发布确认**。  
> **下一步：** 请回复 `测试已通过，可发布`（由本 subagent 更新迭代为「已发布」）

**禁止** 在测试未通过时提示上述话术；**禁止** 用 `技术设计已确认，可开始编码` 代替发布确认。
