...
> **迭代：** `iter-03`（已交付）· **`iter-05`（进行中）** — 见 [iter-05 README](../../iterations/iter-05/README-cn.md)  
> **路线图阶段：** 1 — MVP 聊天 + 配置 UI  
> **状态：** iter-03 已交付 · **iter-05 PRD 已确认**  
> **PRD 确认日期：** 2026-06-16（iter-03）· 2026-06-17（iter-05）  
> **文档版本：** v0.3
...
为已登录用户提供 **Console（控制台）**：左侧菜单 + 多页面。iter-03 交付 **Profile**、**Assistants**、**New Chat 助理选择器**及占位页。**iter-05** 落地 **Models**（BYOK + 测试）、重构 **Profile**（Account / Preferences 双 Card），知识库 / MCP 仍为占位。视觉与首页 **C2 · Electric Ocean** 一致。**用户可见 UI 文案为 English。**
...
| `/console/models` | Model 管理（iter-05） | 需登录 |
...
知识库上传/RAG、MCP 连接、Anthropic/Azure/自定义 OpenAI-compat provider、每助理 model 选择、OAuth、组织/多租户、助理挂载 KB/MCP。
...
| F-24 | Model 管理 | [prd/models-cn.md](./prd/models-cn.md) | iter-05 |
...
| 文档 | 范围 |
|------|------|
| [prd/profile-cn.md](./prd/profile-cn.md) | Account、Preferences（iter-05 修订） |
| [prd/models-cn.md](./prd/models-cn.md) | BYOK、测试、平台默认 |
