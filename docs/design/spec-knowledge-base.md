...
| PRD / 项 | 定稿 |
|----------|------|
| D1 信息架构 | 侧栏 **「知识库」** → **`/console/knowledge`** 列表；点击库进入 **`/console/knowledge/[baseId]`** 文档与索引相关操作。 |
| D2 删除与索引 | 删库 `confirm` 文案须含 **「将删除库内全部文档及已生成向量」**；删文档同理（或简化为「文档及对应向量」）。文档保存后 **同步索引**（见 §6）；失败时文档展示 **索引失败** 状态与错误摘要。 |
| D3 视觉 | 与 **ConsoleConfigForm / PromptTemplatesForm** 一致：`max-w-*` 卡片、`violet` 主按钮、`zinc` 背景与边框、成功/错误提示条。 |
| D4 空状态 | 无库：引导 **新建知识库**；无文档：引导 **添加文本文档**；索引失败：红色或 amber 条 + 可 **重试索引**（按钮由后端提供接口后接）。 |
...
| `/console/knowledge/[baseId]` | 库名称展示与返回列表；文档表格；新建/编辑文档（抽屉或独立表单区）；可选试检索区。 |
...
- 列建议：**名称**、**描述**（可选，截断）、**文档数**、**更新时间**、操作 **进入 / 重命名 / 删除**。  
...
| **文档** | `id`，`base_id`，`title`（可空），`body`，`index_status`（`pending` \| `indexing` \| `ready` \| `failed`），`index_error`（可空），`updated_at` |
| **块** | `id`，`entry_id`，`chunk_index`，`text`，`char_start`，`char_end`（相对文档 `body` 的偏移，便于调试展示） |
| **向量** | 与 **块一行一向量**；存 **维度**、**模型名**、**向量 BLOB**（`float32` 小端序）或后端选定等价存储 |

- **删除库**：级联删除文档、块、向量。  
- **更新文档**：先删该文档下旧块与向量，再分块与重新写入。
...
- **配置**（环境变量，首版）：  
  ...
  - `KNOWLEDGE_EMBEDDING_API_KEY`（必填方可写入向量；缺省时保存文档成功但 `index_status=failed` 并提示）  
  - `KNOWLEDGE_EMBEDDING_MODEL`（默认 `text-embedding-3-small` 或与服务商一致）  
- **批处理**：单文档多块时可 **批量 embedding** 减少请求次数（上限由后端控制）。

### 6.3 向量检索（试检索与后续 RAG 共用）

- **相似度**：**余弦相似度**；向量须 **L2 归一化** 后可用点积（由后端统一约定）。  
- **首版规模**：单机、单库文档有限，允许 **暴力扫全库块向量**；模块内封装 `searchChunks({ baseId, queryVector, topK })` 供试检索与后续对话迭代复用。

### 6.4 索引时机

- **首版**：文档 **创建/更新** 后，在 **同一请求链路内** 完成删旧块 → 分块 → embedding → 写库（避免引入队列复杂度）。若耗时过长可再迭代改为异步 + 轮询状态。  
