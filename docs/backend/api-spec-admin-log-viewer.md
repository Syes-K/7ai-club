...
| 参数 | 必填 | 说明 |
|------|------|------|
| `start` | 与 `end` 成对可选 | ISO 8601，区间**下界**（闭区间） |
| `end` | 与 `start` 成对可选 | ISO 8601，区间**上界**（闭区间） |
...
**时间缺省**：`start` 与 `end` **同时省略**时，使用「**当前时刻起往前 1 小时**」`[now-1h, now]`（闭区间）。若只传其一则 **400**。
...
| 状态 | `code` | 说明 |
|------|--------|------|
| 400 | `LOG_QUERY_RANGE` | 时间只传一半、或 end \< start、或 ISO 无效（实现上对缺省成对已有分支） |
...
与 `GET /api/console/logs` 的 `start` / `end` 规则相同（成对可选、缺省为最近 1 小时）。
