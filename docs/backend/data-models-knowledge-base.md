...
|----|------|------|
| `id` | TEXT PK | UUID |
| `entry_id` | TEXT FK → knowledge_entries.id ON DELETE CASCADE | 所属文档 |
| `chunk_index` | INTEGER NOT NULL | 条内序号 |
| `text` | TEXT NOT NULL | 块文本 |
| `char_start` | INTEGER NOT NULL | 相对文档 `body` 的 UTF-16 偏移起点 |
| `char_end` | INTEGER NOT NULL | 相对文档 `body` 的 UTF-16 偏移终点（开区间约定以实现为准） |
...
- 删库：外键级联删除该库下所有文档、块（含向量）。
- 更新文档正文：先删除该文档下全部 `knowledge_chunks`，再写入新块与向量。
