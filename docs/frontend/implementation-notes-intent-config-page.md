...
1. 知识库文档选项当前通过“先拉 knowledge base，再逐个拉 entries”拼装，知识库规模很大时会增加初始化请求数。
...
2. 将某路由 `nextNodes` 选中 `knowledge_search`，清空知识库文档后点击保存，应被前端阻止并显示字段错误。
3. 补齐知识库文档后点击“校验配置”，应通过；再点击“保存配置”应返回成功并更新版本标识。
