...
- 可在应用配置 `app-config.json` 中通过 `knowledgeChunkSize`、`knowledgeChunkOverlap` 调整（影响条目重建时的切分粒度与冗余）。
...
| `KNOWLEDGE_EMBEDDING_BASE_URL` | 可选；与 `app-config.json` 并存时环境变量优先 |
| `KNOWLEDGE_EMBEDDING_MODEL`    | 可选；与 `app-config.json` 并存时环境变量优先 |

应用配置：`embeddingApiBaseUrl`、`embeddingModel`（可选）。优先级：环境变量 > 应用配置 > 内置默认。API Key 仅 `KNOWLEDGE_EMBEDDING_API_KEY`。
